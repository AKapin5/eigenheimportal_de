<x-slot:title>
    @if ($blogCategory->exists)
        {{ __('Edit blog category ":name"', ['name' => $blogCategory->name]) }}
    @else
...
<form wire:submit.prevent="store()" method="post" enctype="multipart/form-data">
    ...
    <div class="tab-content">
        @foreach($languages as $language)
            <div wire:ignore.self
                ...
                <x-admin.form.input
                    :attribute="'blogCategory.name.' . $language"
                    :model="$blogCategory"
                    :label="__('Name')" />

                <x-admin.form.input
                    :attribute="'blogCategory.alias.' . $language"
                    :model="$blogCategory"
                    :label="__('Alias')" />

                <x-admin.form.input
                    :attribute="'blogCategory.seo_title.' . $language"
                    :model="$blogCategory"
                    :label="__('SEO Title')"/>

                <x-admin.form.textarea
                    :attribute="'blogCategory.seo_description.' . $language"
                    :model="$blogCategory"
                    :label="__('SEO Description')">
                </x-admin.form.textarea>

                <x-admin.form.textarea
                    :attribute="'blogCategory.seo_keywords.' . $language"
                    :model="$blogCategory"
    ...
    <x-admin.form.select
        :type="'select2'"
        :placeholder="''"
        :attribute="'blogCategory.parent_id'"
        :options="$categoryOptions"
        :model="$blogCategory"
        :label="__('Parent')" />

    <x-admin.form.checkbox
        :attribute="'blogCategory.status'"
        :model="$blogCategory"
        :label="__('Show')" />

    <x-admin.form.input
        :attribute="'blogCategory.sort'"
        :model="$blogCategory"
        :label="__('Sort')" />
